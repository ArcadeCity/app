...
import { ACTIVE_OPACITY, color, images, spacing } from 'views/theme'
...
export function MenuButton(props: MenuButtonProps) {
  ...
  return (
    <TouchableOpacity {...(rest as any)} style={setViewStyle} activeOpacity={ACTIVE_OPACITY}>
      <View style={(image || children) && setImageStyle}>
        {image && <Image source={image} style={{ height: 24, width: 24, resizeMode: 'contain' }} />}
        {children}
      </View>
      <View style={{ minHeight: 20 }}>
        {hasTitle && <Text preset='label' tx={titleTx} text={title} />}
        {hasDescription && (
          <Text preset='description' tx={descriptionTx} text={description} style={DESCRIPTION} />
        )}
      </View>
