...
import { ReactThreeFiber, useFrame } from '@react-three/fiber/native'
...
export const Stars = React.forwardRef(
  (
    { radius = 100, depth = 50, count = 5000, saturation = 0, factor = 4, fade = false }: Props,
    ref
  ) => {
    ...
    useFrame(
      (state) =>
        // @ts-ignore
        material.current && (material.current.uniforms.time.value = state.clock.getElapsedTime())
    )
